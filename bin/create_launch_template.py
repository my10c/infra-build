...
def main():
    ...
    print('aws_cfg values {}',config.settings['aws_cfg']['credentials'])
    ...
    for template in templates_data:
        ...
        for tag in template_data['TagSpecifications']:
            ...
            tags_list_volume.append(temp_tag_dict)
            # hardcode to add the Tag Name for volume with the value of component
            if tag == 'component':
                temp_tag_dict = {'Key': 'Name', 'Value': template_data['TagSpecifications'][tag]}
                tags_list_volume.append(temp_tag_dict)
            ...
            result = ec2_conn.create_launch_template(
                ...
                LaunchTemplateData={
                    ...
                    'TagSpecifications': [
                        ...
                        {
                            'ResourceType': 'volume',
                            'Tags':  tags_list_volume
                        }
                    ],
                    'SecurityGroupIds': sgs_list
                }
            )
            print('Launch template {} created.'.format(template))
